...
#include "../robotline_fullspeed.ino"
#include <cstdio>
#include <cstdlib>
#include <cstring>
...
int main(int argc, char** argv) {
  ...
  if (std::strcmp(test, "full_speed") == 0) {
    sample(4); CHECK(left() == 255 && right() > 0);
    sample(7); CHECK(left() == -255 && right() > 0);
    sample(28); CHECK(left() == 255 && right() < 0);
    sample(4); CHECK(left() == 255 && right() > 0);
    ...
    // At full speed the outer wheel is already capped; slowing the inner
    // wheel must still produce right steering, without demanding PWM > 255.
    sample(8); CHECK(left() >= straightL && right() < straightR);
    CHECK(left() * straightR > right() * straightL);
