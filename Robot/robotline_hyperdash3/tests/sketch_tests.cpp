...
#include "../robotline_hyperdash3.ino"
#include <cstdio>
#include <cstdlib>
#include <cstring>
...
int main(int argc, char** argv) {
  ...
  if (std::strcmp(test, "unsupported_power") == 0) {
    // A rejected power setup must leave both bridge enables off, even at STOP.
    CHECK(pwm[5] == 0 && pwm[10] == 0);
    for (int mask = 0; mask < 32; ++mask) {
      run(mask, 100);
      CHECK(pwm[5] == 0 && pwm[10] == 0);
      CHECK(direction[6] == LOW && direction[7] == LOW);
      CHECK(direction[9] == LOW && direction[8] == LOW);
    }
    Serial.input = "STOP\nSTART\n"; run(4, 100);
    CHECK(pwm[5] == 0 && pwm[10] == 0);
    CHECK(Serial.output.find("MOTOR_POWER_CONFIG") != std::string::npos);
    CHECK(Serial.output.find("RAW=") != std::string::npos);
  } else if (std::strcmp(test, "run_profile") == 0) {
    // The downloaded profile must start without Serial or a configuration edit.
    Serial.connected = false;
    sample(4);
    CHECK(left() > 0 && right() > 0);
    CHECK(!stopped && std::strcmp(state, "FOLLOW") == 0);
    const int masks[] = {2, 6, 8, 12, 1, 16, 7, 15, 28, 30, 4, 31};
    for (int mask : masks) {
      sample(mask);
      CHECK(std::abs(left()) <= MAX_SPEED && std::abs(right()) <= MAX_SPEED);
      CHECK(left() != 0 || right() != 0);
    }
    // Lost-line recovery also stays within the trial drive duty limit.
    sample(4); sample(0);
    run(0, 700);
    CHECK(left() < 0 && right() < 0);
    CHECK(std::abs(left()) <= MAX_SPEED && std::abs(right()) <= MAX_SPEED);
    ...
    run(4, 200); braked();
    CHECK(Serial.output.find("MOTOR_POWER_CONFIG") == std::string::npos);
  } else if (std::strcmp(test, "steering") == 0) {
    ...
    sample(1); CHECK(left() < 0 && right() > 0);
    sample(16); CHECK(left() > 0 && right() < 0);
    sample(7); CHECK(left() < 0 && right() > 0);
    sample(28); CHECK(left() > 0 && right() < 0);
    sample(4); CHECK(left() > 0 && right() > 0);
    ...
    sample(4); CHECK(left() > 0 && right() > 0);
    sample(8); sample(0); CHECK(left() > 0 && right() < 0);
    run(0, 650); braked();
    ...
    sample(1); run(1, 1900); CHECK(left() < 0 && right() < 0);
    sample(2); braked();
    ...
    sample(8); CHECK(left() > right() && Serial.input.size() >= 984);
    Serial.input = "\nSTOP"; loop(); braked();
  } else {
    CHECK(false);
  }
